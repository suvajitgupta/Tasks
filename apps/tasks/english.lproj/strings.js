// Place strings you want to localize here.  In your app, use the key and
// localize it using "key string": "text".  HINT: For your key names, use the
// English string with an underscore in front.  This way you can still see
// how your UI will look and you'll notice right away when something needs a
// localized string added to this file!
//

/** @class

  @version 0.1
  @author Suvajit Gupta
*/
SC.stringsFor('English', {

  "_TASKS": "TASKS",
  "_TEAM": "TEAM",
  "_ActionsButtonTooltip": "Globally available actions",
  "_Activated:": "Activated: ",
  "_ActivatedAtTooltip": "Project activation or start date",
  "_Active": "Active",
  "_Add": "Add",
  "_AddProjectTooltip": "Add a new project",
  "_AddTaskTooltip": "Add a new task, to same project/assignee if there is a selected task",
  "_AddUserTooltip": "Add a new user",
  "_AllTasks": "All Tasks",
  "_Apply": "Apply",
  "_AreYouSure": "Are you sure?",
  "_Assignee:": "Assignee: ",
  "_AssigneeNotLoaded": "Not loaded",
  "_AssigneeOverloaded": "Overloaded",
  "_AssigneeProperlyLoaded": "Properly loaded",
  "_AssigneeUnderLoaded": "Under loaded",
  "_Assignees:": "Assignees: ",
  "_Authenticating": "Authenticating...",
  "_AutoSave": "Auto Save",
  "_Bug": "Bug",
  "_ByAnyone": "By Anyone",
  "_ByMe": "By Me",
  "_Cancel": "Cancel",
  "_ChooseDate": "Choose Date",
  "_ClickToViewEditDetailsTooltip": "Click to view/edit details",
  "_ClippyTooltip": "Copy ID/Name of selected tasks to clipboard",
  "_Close": "Close",
  "_Comment": "Comment",
  "_CommentTooltip": "Comment on this task",
  "_Completed": "Completed",
  "_CompletedTooltip": "Ready for Primetime: what's done & passed",
  "_Confirmation": "Confirmation",
  "_Copy": ": copy",
  "_CopyID/Name": "Copy ID/Name",
  "_CopyLinkLocation": "Copy Link Location",
  "_CreateMissingUsers": "Create Missing Users",
  "_CreateMissingUsersOnscreenHelp": "Automatically create new users for unknown assignees/submitters during import",
  "_Created:": "Created: ",
  "_Credits": "Credits: Suvajit Gupta, Sean Eidemiller, Josh Holt, and Matt Grantham",
  "_CommentDeletionConfirmation": "Are you sure you want to delete the selected comment?",
  "_Delete": "Delete",
  "_DeleteComment": "Delete the comment below",
  "_DeleteProjectTooltip": "Delete selected projects",
  "_DeleteTaskTooltip": "Delete selected tasks",
  "_DeleteUserTooltip": "Delete selected users",
  "_Description:": "Description:",
  "_DescriptionHint": "Enter detailed notes...",
  "_Description": "Description",
  "_Developer": "Developer",
  "_DisplayModeTooltip": "Toggle Display Mode between TEAM and TASKS",
  "_DisplayedUsers": " users displayed, ",
  "_Done": "Done",
  "_DoneEffortRangeWarning": " (completed task shouldn't have an effort range)",
  "_DontCare": "Don't Care",
  "_Duplicate": "Duplicate",
  "_EditComment": "Edit the comment below",
  "_Effort:": "Effort:",
  "_EffortOnscreenHelp": "May be unspecified, a decimal like '0.25',<br> or a range like '2-3'; in units 'd' or 'h'",
  "_EffortSpecified:": "Effort Specified:",
  "_Email:": "Email:",
  "_EmailAddress": "Your Email Address",
  "_EmailOnscreenHelp": "Specify to get gravatar &amp; notifications",
  "_Export": "Export",
  "_Failed": "Failed",
  "_Feature": "Feature",
  "_FilterTooltip": "Filter tasks by type, priority, status, validation, effort specified, recently updated",
  "_FirstnameLastname": "Firstname Lastname",
  "_FullName:": "Full Name:",
  "_GotoNextTask": "Go to next task",
  "_GotoPreviousTask": "Go to previous task",
  "_Graphical": "Graphical",
  "_Gravatar:": "Gravatar:",
  "_Guest": "Guest",
  "_GuestSignUp": "Guest Sign Up",
  "_Has": "Has ",
  "_Hi": "Hi ",
  "_High": "High",
  "_ID": "ID: ",
  "_Import": "Import",
  "_ImportDataSoftwareMode": "[User LoginName] (Name) {Role} <Email>\nProject Name {TimeLeft} <ActivatedAt> @Status\n- Task Name {Effort} <Submitter> [Assignee] $Type @Status %Validation\n| Description (1 or more lines following a Project or Task)",
  "_ImportDataTodoMode": "[User LoginName] (Name) {Role} <Email>\nProject Name {TimeLeft} <ActivatedAt> @Status\n- Task Name {Effort} <Submitter> [Assignee] @Status\n| Description (1 or more lines following a Project or Task)",
  "_ImportInstructions:": "Paste or type in data to be imported:",
  "_IncompleteEffortWarning": " (total effort incomplete)",
  "_Initials": "Initials",
  "_InvalidEmailAddress": "This email address",
  "_InUse": "In Use",
  "_Invalid": "Invalid",
  "_LaunchExportHTML": "Export (HTML)...",
  "_LaunchExportText": "Export (Text)...",
  "_LaunchHelp": "Help...",
  "_LaunchImport": "Import...",
  "_LaunchSettings": "Users/Settings...",
  "_LaunchStatistics": "Statistics...",
  "_Loaded": "Loaded ",
  "_Loading": "Loading...",
  "_LoadDoneProjectData": "Load data for completed projects",
  "_LoadFailed": "Load failed ",
  "_LoginServerAccessError": "Login failed - cannot access server",
  "_LoginAuthenticationError": "Login failed - please try again",
  "_LoginName:": "Login Name:",
  "_LoginSince": "Click to see your tasks; Logged in since ",
  "_Logout": "Logout",
  "_LogoutConfirmation": "Are you sure you want to log out from the application?",
  "_Low": "Low",
  "_Manager": "Manager",
  "_Medium": "Medium",
  "_Name": "Name:",
  "_NewComment": "New Comment",
  "_NewProject": "New Project",
  "_NewTask": "New Task",
  "_No": "No",
  "_NoFilter": "No Filter",
  "_NoFilterTooltip": "Clear tasks filter",
  "_Numerical": "Numerical",
  "_OnlineHelp": "Online Help",
  "_Other": "Other",
  "_Overview": "Overview",
  "_Passed": "Passed",
  "_Password:": "Password:",
  "_PasswordHint": "Password",
  "_PasswordTooltip": "Has a password",
  "_Planned": "Planned",
  "_Priority": "Priority:",
  "_PriorityTooltip": "Importance of task: must do, plan to do, do if you can",
  "_Project": "Project",
  "_Project:": "Project: ",
  "_ProjectDeletionConfirmation": "Are you sure you want to delete the selected projects?",
  "_ProjectDeletionConsequences": "Any tasks allocated to deleted projects will become unallocated",
  "_ProjectCountdownTooltip": "Countdown of days left",
  "_Project": "Project ",
  "_Projects": "Projects",
  "_QuickFilters": "Quick Filters",
  "_QuickFiltersTooltip": "Commonly used filters",
  "_RecentlyUpdated:": "Recently Updated:",
  "_RecentlyUpdatedTooltip": "Dot indicates an item recently created/updated (within last day)",
  "_RedFlags:": "Red Flags: ",
  "_RefreshTooltip": "Reload latest Tasks data from the server",
  "_Risky": "Risky",
  "_Role:": "Role:",
  "_Saved": "Saved ",
  "_SaveConfirmation": "You have unsaved changes - save before logging out?",
  "_SaveError": "Save failed ",
  "_SaveTooltip": "Save modified Tasks data to the server",
  "_SendNotifications": "Send Notifications",
  "_Showstoppers": "Showstoppers",
  "_ShowstoppersTooltip": "Are We There Yet: open high priority defects",
  "_ShowProjectsList": "Show Projects List",
  "_ShowTasksList": "Show tasks list",
  "_SignIn": "Sign In",
  "_SignUp": "Sign Up",
  "_Statistics": "Statistics",
  "_Status": "Status:",
  "_StatusTooltip": "Development status of task: show progress or obstacles",
  "_Stopped:": "OR Stopped:",
  "_StoppedTooltip": "Project end date - this will set the time left",
  "_Submitter:": "Submitter: ",
  "_SubmitterTooltip": "; Submitted by ",
  "_Submitters:": "Submitters: ",
  "_TasksSummary:": "Tasks Summary: ",
  "_System": "System",
  "_TargetDate:": "Target Date:",
  "_TaskDeletionConfirmation": "Are you sure you want to delete the selected tasks?",
  "_TaskDeletionConsequences": "Comments/watches associated with deleted tasks will also be deleted",
  "_TaskEffortTooltip": "Task effort",
  "_TaskIdTooltip": "Task #ID (dashes indicate unsaved, underline indicates watch)",
  "_TaskValidationTooltip": ", background color shows validation",
  "_Tasks": "Tasks",
  "_Task": "Task",
  "_TaskWatchersTooltip": "Number of users watching this task - click to see them",
  "_TasksSearchHint": "Tasks search pattern",
  "_TasksSearchTooltip": "Specify @Users (Assignees or Submitters) or [Assignees] or <Submitters> (use all or part of full/login name separated via commas or spaces - use 'none' for unassigned), #IDs (separated by commas or spaces), or text (to match within task name or description - prefix with a caret to find items that don't match text)",
  "_Tester": "Tester",
  "_TimeLeft:": "Time Left:",
  "_TimeLeftOnscreenHelp": "May be unspecified, a decimal like '0.25'; in units 'd' or 'h'",
  "_Troubled": "Troubled",
  "_TroubledTooltip": "Manage by Exception: what's risky or failed",
  "_Type": "Type:",
  "_TypeTooltip": "Kind of task: user visible functionality, defect fix, or other",
  "_UnallocatedTasks": "Unallocated Tasks",
  "_UnassignedTasks": "Unassigned Tasks",
  "_Unassigned": "Unassigned",
  "_UnavailableRoute": "This route is not available when logged in",
  "_Unfinished": "Unfinished",
  "_UnfinishedTooltip": "Deliver on Commitments: what's left to finish",
  "_UnimplementedFunctionality": "Unimplemented Functionality: ",
  "_Untested": "Untested",
  "_Unvalidated": "Unvalidated",
  "_UnvalidatedTooltip": "Trust but Verify: what's left to validate",
  "_Unwatch": "Unwatch",
  "_Updated:": "Updated: ",
  "_Upcoming": "Upcoming",
  "_UpcomingTooltip": "Next Up: unfinished medium/low priority tasks",
  "_Urgent": "Urgent",
  "_UrgentTooltip": "Immediate Followup Needed: unfinished high priority tasks",
  "_User": "User",
  "_UserDeletionConfirmation": "Are you sure you want to delete the selected users?",
  "_UserDeletionConsequences": "Tasks assigned to deleted users will become unassigned and associated comments/watches deleted",
  "_UsersSettings": "User Info/Settings",
  "_LoginNameHint": "Login Name",
  "_UserSearchHint": "User name/email pattern",
  "_UserSearchTooltip": "Specify pattern to match all or part of users' full/login name or email",
  "_Validation": "Validation:",
  "_ValidationTooltip": "Testing status of completed task - independent verification recommended",
  "_Watch": "Watch",
  "_Watched:": "Watched:",
  "_Yes": "Yes",
  "_assignees": " assignees & ",
  "_at": " at ",
  "_commented": "commented ",
  "_daysAgo": " days ago",
  "_description": "description",
  "_displayed": " displayed",
  "_finished": " finished",
  "_hoursAgo": " hours ago",
  "_justNow": "just now",
  "_left": " left",
  "_minutesAgo": " minutes ago",
  "_of": " of ",
  "_oneHourAgo": " 1 hour ago",
  "_projects": " projects & ",
  "_redFlags": " red flags",
  "_selected": " selected",
  "_tasks": " tasks",
  "_total": ", total ",
  "_users": " users",
  "_with": " with ",
  "_yesterday": "yesterday"
  
});
